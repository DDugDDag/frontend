...
class SearchService {
  private kakaoRestApiKey = Constants.expoConfig?.extra?.KAKAO_REST_API_KEY as
    | string
    | undefined;
  ...
  async searchIntegrated(
    ...
    try {
      ...
      if (placesResponse.data) {
        const placeSuggestions: SearchSuggestion[] = placesResponse.data.map(
          (place) => ({
            id: place.id,
            name: place.title,
      ...
      // 중복 제거 및 관련성 순으로 정렬
      const uniqueSuggestions = this.removeDuplicatesAndSort(
        suggestions,
        query
      );
  ...
  private determineLocationType(
    placeName: string
  ): "location" | "station" | "landmark" {
    const lowerName = placeName.toLowerCase();

    if (
      lowerName.includes("역") ||
      lowerName.includes("station") ||
      lowerName.includes("대여소")
    ) {
      return "station";
    }

    if (
      lowerName.includes("공원") ||
      lowerName.includes("산") ||
      lowerName.includes("강") ||
      lowerName.includes("박물관") ||
      lowerName.includes("기념관") ||
      lowerName.includes("타워") ||
      lowerName.includes("궁") ||
      lowerName.includes("사") ||
      lowerName.includes("절")
    ) {
      return "landmark";
    }

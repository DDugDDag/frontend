...
interface AppContextType {
  ...
  actions: {
    ...
    stopNavigation: () => void;
    updateNavigation: (
      info: Partial<AppState["map"]["navigationInfo"]>
    ) => void;

    // 대여소 액션
    loadStations: (stations: BikeStation[]) => void;
    setNearbyStations: (stations: BikeStation[]) => void;
    selectStation: (station: BikeStation | null) => void;
...
export const AppProvider: React.FC<AppProviderProps> = ({ children }) => {
  ...
  const actions = {
    ...
    setSelectedLocation: (
      ...
      dispatch({
        ...
        payload: { lat, lng, address, name },
      });
    },

    setRoute: (route: RouteResponse) => {
    ...
    // 대여소 액션
    loadStations: (stations: BikeStation[]) => {
      dispatch({ type: "STATIONS_LOAD_SUCCESS", payload: stations });
    },

    setNearbyStations: (stations: BikeStation[]) => {
      dispatch({ type: "STATIONS_SET_NEARBY", payload: stations });
    },

    selectStation: (station: BikeStation | null) => {
  ...
  const contextValue: AppContextType = { state, dispatch, actions };
